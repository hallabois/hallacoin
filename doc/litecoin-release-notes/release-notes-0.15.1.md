Hallacoin Core version *0.15.1* is now available from:

  <https://download.Hallacoin.org/Hallacoin-0.15.1/>
...
A number of changes to the way Hallacoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 laudaatturit per kB (0.00001 HLA). This is to prepare for dropping the minimum transaction fee to 0.1 laudaatturit per kB (0.0001 HLA) in the next release.
