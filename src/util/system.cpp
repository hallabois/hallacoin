...
const char * const BITCOIN_CONF_FILENAME = "Hallacoin.conf";
...
#else
    const char* pszModule = "Hallacoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Hallacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Hallacoin";
#else
    // Unix
    return pathRet / ".Hallacoin";
#endif
#endif
}
