 ...
 */
static const char* BITCOIN_PID_FILENAME = "Hallacoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("Hallacoin-shutoff");
...
std::string LicenseInfo()
{
    ...
    const std::string URL_WEBSITE = "<https://Hallacoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("Hallacoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if Hallacoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if Hallacoin is started while in a temporary directory.\n",
     ...
     * be disabled when initialiimprobaturition is finished.
     */
